<template>
  ...
  <v-expansion-panels class="brand-list" accordion  expand-icon="mdi-menu-down">
    <v-expansion-panel>
      ...
      <v-expansion-panel-content>
            <ul>
                <li v-for="(color,index) in brand_colors" :key="color.id" class="py-3 mt-2 d-flex color-item" :class="{active:color.active}" @click="setColorElements(color,index);getSearchProduct()">
                  <div 
  ...
  <v-expansion-panels class="brand-list" accordion  expand-icon="mdi-menu-down">
    <v-expansion-panel>
      ...
      <v-expansion-panel-content>
         <ul>
                <li v-for="item in brand_delivery" :key="item.id" @click="getSearchProduct()" class="py-2 d-flex brand-item">
                  <v-checkbox
                    hide-details
                    ...
                    class="shrink mr-2 mt-0"
                  ></v-checkbox>
                  <a href=""><span></span>{{item.name}} ()</a></li>
          </ul>
      </v-expansion-panel-content>
    </v-expansion-panel>
  </v-expansion-panels>
  ...
  <v-expansion-panels class="brand-list" accordion  expand-icon="mdi-menu-down">
    <v-expansion-panel>
      ...
      <v-expansion-panel-content>
        <ul>
                <li v-for="index in range" :key="index" class="py-2 d-flex brand-item">
                  <v-checkbox
          hide-details
          class="shrink mr-2 mt-0"
        ></v-checkbox>
        <a v-if="index == 1" href=""><span></span>${{min}} - ${{Math.ceil(max / index)}}</a>
        <a v-else href=""><span></span>${{Math.ceil(max / (index-1))}} - ${{Math.ceil(max / index)}}</a>
...
export default {
  ...
  computed: {
    ...
    ...mapState("product", ["brand_products"]),
    ...mapState("brand", ["brand_colors"]),
    ...mapState("brand", ["brand_delivery"]),
    ...mapState("brand", ["brand_price"]),
  },
  methods: {
    openPanel(index){
    ...
    {
       let new_color = this.category_colors[index];
      color.active = !color.active;
      this.$set(this.category_colors, index, new_color);
      ...
      console.log(this.color_items)
    },
    getBrandProducts()
    {
      this.apiArg.brand_id = this.$route.params.id
      this.$store
        .dispatch("product/getBrandProducts",this.apiArg)
        .then(() => {
          this.Searchproducts = []
          this.Searchproducts = this.$store.getters["product/brand_products"]
    ...
    brandColors(){
      this.$store
        .dispatch("brand/getBrandColors",this.$route.params.id)
        .then(() => {})
        .catch(() => {});
    },
    brandDelivery(){
      this.$store
        .dispatch("brand/getBrandDelivery",this.$route.params.id)
        .then(() => {})
        .catch(() => {});
    },
    getSearchProduct()
    {
      ...
      console.log("products list",this.brand_products)
      // console.log("brand item length",this.brands.length)
      console.log("Search products",this.Searchproducts)
      console.log("Delivery Types",this.Searchproducts)
      let i = 0;
      if(this.delivery_types.length > 0 && this.color_items.length > 0){
        this.Searchproducts = this.$store.getters["product/brand_products"]
      }
    ...
    brandPrice()
    {
      axios.post('api/product/get_brand_products',{
        ...
        this.$store
        .dispatch("brand/getBrandPrice",data)
        .then(() => {})
        .catch(() => {});
        })
