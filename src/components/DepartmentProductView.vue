<template>
  <div class="col-lg-9 mt-4">
    ...
    <div class="right-side">
      <div
        class="product-grid"
        v-for="(product, index) in department_products"
        :key="index"
      >
...
export default {
  ...
  async created() {
      await this.departmentProduct();
      this.getCarts();
  },
  computed: {
    ...mapState("product", ["department_products"]),
    ...mapState("cart", ["carts"]),
    // ...mapState("product", ["color_images"]),
  },
  methods: {
    ...mapActions("cart", ["getCarts"]),
    async departmentProduct() {
      this.$store
        .dispatch("product/getDepartmentProducts", {
